...
passkeys (Touch ID, Face ID, or a hardware key), with a break-glass password as the recovery
path of last resort.
...
**`KEYSTATIC_AUTH_MODE` defaults to `basic`**, exactly the old behaviour. Only the
...
| Var                        | Required | Notes                                                                                           |
| -------------------------- | -------- | ----------------------------------------------------------------------------------------------- |
| `KEYSTATIC_SESSION_SECRET` | yes      | `openssl rand -hex 32`. Missing means 503 on all `/keystatic`. Rotating it kills every session. |
...
1. Unlock at `/auth/keystatic` with a passkey, or the break-glass password if no passkey exists yet.
